...
void ocl_init (void)
{
    ...
    compute_kernel_optimized = clCreateKernel (program, kernel_optimized_name, &err);
    check (err, "Failed to create compute kernel");

    printf ("Using kernel for optimized: %s\n", kernel_optimized_name);
...
unsigned ocl_compute_optimized (unsigned nb_iter) {
    size_t global[2] = { SIZE, SIZE };  // global domain size for our calculation
    size_t local[2]  = { TILEX, TILEY };  // local domain size for our calculation
